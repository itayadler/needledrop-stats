...
async function createDb(dbPath) {
  return new Promise((resolve, reject) => {
    ...
    if (!existsSync(dbPath)) {
      db.serialize(() => {
        db.run(
          "CREATE TABLE IF NOT EXISTS reviews (youtube_id VARCHAR(20), review_name VARCHAR(255), created_at VARCHAR(255), review_url VARCHAR(2048), youtube_description TEXT, rank REAL, artist_name VARCHAR(255), album_name VARCHAR(255))"
        );
        db.run(
          "CREATE UNIQUE INDEX IF NOT EXISTS review_youtube_id_index ON reviews(youtube_id)"
        );
        ...
        db.run(
          "alter table reviews add column spotify_artist_url varchar(1024)"
        );
        db.run(
          "alter table reviews add column spotify_artist_image varchar(1024)"
        );
